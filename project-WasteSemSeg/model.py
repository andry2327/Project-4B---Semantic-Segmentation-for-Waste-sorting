...
class InitialBlock(nn.Module):
    ...
    - input(Tensor): A 4D tensor of shape [batch_size, channel, height, width]
        ...
        input tensor: 'batch_size' batches, where each batch is an image: 3 channels (R,G,B), height and width
    '''

    def __init__(self):
        super(InitialBlock, self).__init__()
        self.conv = nn.Conv2d(3, 13, (3, 3), stride=2, padding=1) # 3 input channels (RGB), 13 output channels (13 filters), filter size = 3x3, stride = 2
    ...
    '''
    INPUT -> conv -> batch_norm -> -> PReLU -> OUT2
    INPUT -> MaxPool -> OUT1                                 ---> OUTPUT_stage1 = torch.cat(OUT1, OUT2) 
    
    -> OUTPUT_stage1.shape = (16, 256, 256) // 256 beacuse of maxPooling with 2 x 2 windows: 512 -> 256
    '''
